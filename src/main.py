...
history_len = 3
list_name = f"0710_{history_len}"

# print("loading faiss_list_e...")
# faiss_list_e = pickle.load(open(f"fassi_entity_time_emb_label_{list_name}.pkl", "rb"))
# faiss_list_e = faiss_list_e.reshape(-1, 402)
# print("loading faiss_list_r...")
# faiss_list_r = pickle.load(open(f"fassi_rel_time_emb_label_{list_name}.pkl", "rb"))
# faiss_list_r = faiss_list_r.reshape(-1, 402)
# print("loading index_entity...")
...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, mode):
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        if time_idx - args.test_history_len<0:
            input_list = history_list[-args.test_history_len+time_idx:]+test_list[0:time_idx]
        else:
            input_list = test_list[time_idx - args.test_history_len:
                                    time_idx]

        history_glist = [build_sub_graph(num_nodes, num_rels, g, use_cuda, args.gpu) for g in input_list]
        test_triples_input = torch.LongTensor(test_snap).cuda() if use_cuda else torch.LongTensor(test_snap)
        test_triples_input = test_triples_input.to(args.gpu)
        test_triples, final_score, final_r_score = model.predict(history_glist, num_rels, static_graph, test_triples_input, use_cuda, None, faiss_list_e, faiss_list_r, index_entity, index_rel, None)
...
def run_experiment(args, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    num = 0
    for day in all_ans_list_r_test:
        for s ,o in day.items():
            for object, r_set in o.items():
                num += len(r_set)
    ...
    if args.test and os.path.exists(model_state_file):
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                loss_e, loss_r, loss_static = model.get_loss(history_glist, output[0], static_graph, use_cuda, train_sample_num, faiss_list_e, faiss_list_r, index_entity, index_rel, epoch if index_rel==None else None)
            ...
            if epoch and epoch % args.evaluate_every == 0:
                mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r = test(model, 
                                                                    ...
                                                                    mode="train")
                if use_aim:
                    run.track(mrr_raw, name='mrr_raw', step=epoch, context={ "subset":"test" })
                    run.track(mrr_raw_r, name='mrr_raw_r', step=epoch, context={ "subset":"test" })
        ...
        mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r = test(model, 
                                                            ...
                                                            static_graph, 
                                                            mode="test")
        if index_entity == None:
            fassi_insert_list_entity = np.concatenate(faiss_list_e)
            fassi_insert_list_rel = np.concatenate(faiss_list_r)
            pickle.dump(fassi_insert_list_entity, open(f"fassi_entity_time_emb_label_{list_name}.pkl", "wb"))
            pickle.dump(fassi_insert_list_rel, open(f"fassi_rel_time_emb_label_{list_name}.pkl", "wb"))
    return mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r
...
if __name__ == '__main__':
    ...
    args.n_hidden = 200
    args.self_loop = True
    args.decoder = "convtranse"
    args.encoder = "uvrgcn"
    args.layer_norm = True
    args.weight = 0.5
    args.entity_prediction = True
    args.relation_prediction = True
    args.add_static_graph = True
    args.angle = 10
    args.discount = 1
    ...
    if use_aim:
        ...
        run["hparams"] = {
            ...
            "train_history_len": args.train_history_len,
