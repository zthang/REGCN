...
class RecurrentRGCN(nn.Module):
    ...
    def forward(self, g_list, static_graph, use_cuda):
        ...
        for i, g in enumerate(g_list):
            g = g.to(self.gpu)
            g.r_to_e = torch.from_numpy(np.array(g.r_to_e))
    ...
    def predict(self, test_graph, num_rels, static_graph, test_triplets, use_cuda, train_sample_num, faiss_list_e, faiss_list_r, index_entity, index_rel, epoch):
        with torch.no_grad():
            ...
            score = self.decoder_ob.forward(embedding, r_emb, all_triples, train_sample_num, faiss_list_e, index_entity, epoch, mode="test")
            score_rel = self.rdecoder.forward(embedding, r_emb, all_triples, train_sample_num, faiss_list_r, index_rel, epoch, mode="test")
            return all_triples, score, score_rel


    def get_loss(self, glist, triples, static_graph, use_cuda, train_sample_num, faiss_list_e, faiss_list_r, index_entity, index_rel, epoch):
        """
        :param glist:
        :param triplets:
        :param static_graph: 
        ...
        if self.entity_prediction:
            scores_ob = self.decoder_ob.forward(pre_emb, r_emb, all_triples, train_sample_num, faiss_list_e, index_entity,  epoch).view(-1, self.num_ents)
            loss_ent += self.loss_e(scores_ob, all_triples[:, 2])
     
        if self.relation_prediction:
            score_rel = self.rdecoder.forward(pre_emb, r_emb, all_triples, train_sample_num, faiss_list_r, index_rel, epoch, mode="train").view(-1, 2 * self.num_rels)
